...
import { SystemEvent, EventType } from './UsageTelemetry';
import {
  IHttpClient,
  IHttpHeader,
  IHttpClientOption,
...
import { ITelemetryClient, CustomProperties } from './ITelemetryClient';
  ...
  public async request<T>(
    ...
    return new Promise(async (resolve, reject): Promise<void> => {
      ...
      if (!this.options.disableCorrelationId && !options?.disableCorrelationId) {
        correlationHeader[request.correlationIdHeaderName || this.options.correlationIdHeaderName || 'CorrelationId'] =
          correlationId;
      }
      ...
      try {
        ...
        if (ex.response) {
          ...
          this.telemetryClient.trackTrace({
            message: 'APIFailedResponseReceived',
          ...
          this.telemetryClient.trackTrace({
            message: 'APIFailedWithoutResponse',
          ...
          reject({
            status: -1,
            statusText: 'APIFailedWithoutResponse',
            data: ex,
          });
        }
      }
